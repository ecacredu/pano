...
@Component({
    selector: 'panorama-international',
    templateUrl: 'international.component.html',
    styleUrls: ['international.component.css']
})

export class PanoramaInternationalComponent  {

    @ViewChild('lgModal') public lgModal: ModalDirective;

    public variable:String;
    public allInters:any=[];
    ...
    constructor(private router: Router,private userService: UserService, private route: ActivatedRoute,public ss:SecureService,public storage:LocalStorageService){
      ...
      //console.log(this.storage.retrieve('activeUser'));
       this.userService.getAllInterPackages().subscribe((res)=>{
          if(res.status==200){
              this.allInters=this.userService.allInterPackages;
          }
      });
    }
    ...
    toShowTitle(type:any){
        let count:number=0;
        for(var i=0; i < this.allInters.length ; i++){
          if(this.allInters[i].packagetype==type){
            count++;
            return true;
          }
        }
